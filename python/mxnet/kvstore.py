...
class KVStore(object):
    ...
    def cpweight(self, key, out=None, priority=0):
        ...
        if use_str_keys:
            check_call(_LIB.MXKVStorePullExLocal(self.handle, mx_uint(len(ckeys)), ckeys, cvals, ctypes.c_int(priority)))
        else:
            check_call(_LIB.MXKVStorePullLocal(self.handle, mx_uint(len(ckeys)), ckeys, cvals, ctypes.c_int(priority)))
    ...
    def set_optimizer(self, optimizer, optimizer_local = 'sgd'):
        ...
        """
        #add this 8.12: if update_local = 1, it means dist is not supported
        update_local = 0
        ...
        if 'dist' in self.type and is_worker.value: # pylint: disable=unsupported-membership-test
            ...
            update_local = 1
        if update_local == 0:
            self._set_updater((opt.get_updater(optimizer_local)))

    # edit this to register the local updater 1.4
