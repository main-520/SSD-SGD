...
from ..ndarray import (adc_sgd_update, ms_update, dc_sgd_update)
# add this 1.4
...
# This optimizer define the DCASGD and implement the calculation using C++
@register
class CPPDCASGD(Optimizer):
    """The DCASGD optimizer.
    ...
    """
    def __init__(self, momentum=0.0, lamda=0.04, **kwargs):
        super(CPPDCASGD, self).__init__(**kwargs)
        self.momentum = momentum
        self.weight_previous = {}
        self.lamda = lamda
    ...
    def update(self, index, weight, grad, state):
        ...
        inter, previous_weight = state
        dc_sgd_update(weight, grad, previous_weight, out = inter, lr = lr, wd = wd, lam = self.lamda)
        previous_weight[:] = weight
        weight[:] += inter

# add this: define the adaptative DCASGD optimizer using python
@register
class ADCASGD(Optimizer):
    """The DCASGD optimizer.
    ...
    lamda : float, optional
       Scale DC value.
    """
    def __init__(self, momentum=0.0, lamda=4, m = 0, **kwargs):
        super(ADCASGD, self).__init__(**kwargs)
        self.momentum = momentum
        self.weight_previous = {}
        self.lamda = lamda
        self.keys = [] #keep index of the weights
        self.MS = {} # dict to keep the index and corresponing grad
        self.m = m #m is a constant instead of the momentum;
        self.local_num = 0 # used for several steps delay
    ...
    def update(self, index, weight, grad, state):
        ...
        # calculating the new lamda
        if index not in self.keys:
            self.keys.append(index)
            self.MS[index] = 0.0

        self.MS[index] = self.m * self.MS[index] + (1-self.m) * grad * grad + 0.0000001
        dive = pow(self.MS[index],0.5)
        #        dive = numpy.sqrt(self.MS[index])
        ...
        if state[0] is not None:
            ...
            mom[:] *= self.momentum
            if self.local_num % 2 == 0:
                mom[:] += -lr * (grad + wd * weight + self.lamda/dive \
                                 * grad * grad * (previous_weight-weight))
            else:
                mom = -lr *(grad + wd * weight + self.lamda/dive \
                             * grad * grad * (previous_weight-weight))
        else:
...
# add this to implement the adaptative DCASGD using CPP
@register
class CPPADCASGD(Optimizer):
    """The ADCASGD optimizer.
    ...
    lamda : float, optional
       Scale DC value.
    """
    def __init__(self, momentum=0.0, lamda=2, m = 0.0, **kwargs):
        super(CPPADCASGD, self).__init__(**kwargs)
        self.momentum = momentum
        self.weight_previous = {}
        self.lamda = lamda
        self.keys = [] #keep keys of the weights
        self.MS = {} # this is a dictory
        self.m = m #m is a constant instead of the momentum;
        self.local_num = 0 # for several steps delay, add this;

    def create_state(self, index, weight):
        return (zeros(weight.shape, weight.context, dtype=weight.dtype), # the mid mom
                weight.copy()) # previous weight
    ...
    def update(self, index, weight, grad, state):
        ...
        if index not in self.keys:
            self.keys.append(index)
            #            self.MS[index] = 0.0
            self.MS[index] = zeros(weight.shape, weight.context, dtype=weight.dtype)

        inter, previous_weight = state
        # ms_update is used for computing the MeanSquare
        #        ms_update(self.MS[index], grad, out = self.MS[index], m = self.m, **kwargs)
        #        self.MS[index] = self.m * self.MS[index] + (1-self.m) * grad * grad + 0.0000001
        ms_update(self.MS[index], grad, out = self.MS[index], m = self.m)
        adc_sgd_update(self.MS[index], weight, grad, previous_weight, out = inter, lr = lr, wd = wd, lam = self.lamda, **kwargs)
        if self.local_num % 2 == 0:
            previous_weight[:] = weight
        self.local_num += 1
#        previous_weight[:] = weight
...
@register
class COSDCASGD(Optimizer):
    def __init__(self, momentum=0.0, lamda=0.04, **kwargs):
        super(COSDCASGD, self).__init__(**kwargs)
        self.momentum = momentum
        self.weight_previous = {}
        self.lamda = lamda
        self.local_num = 0
    ...
    def update(self, index, weight, grad, state):
        ...
        mom, previous_weight, previous_weight_s = state

#        grad_sync = 1.0*((1-lr*wd)*previous_weight_s - weight)/(4.0)
        grad_sync = 1.0*((1-lr*wd)*previous_weight_s - weight)/(8.0) # Equal to w(t+1)-w(t-1)/2

        if mom:
            mom[:] *= self.momentum
            if self.local_num % 1 == 0:
                mom[:] += -lr * (2.0*grad + wd * weight + 0.5*grad_sync)
            else:
                mom[:] += -lr *0.5* (2.0*grad + wd * weight + 0.5*grad_sync)
        else:
            assert(self.momentum == 0.0)
            if self.local_num % 1 == 0:
                mom = -lr * (2.0*grad + wd * weight + 0.5*grad_sync)
                ...
                mom = -lr * 0.5*(2.0*grad + wd * weight + 0.5*grad_sync)
#        previous_weight_s[:] = weight
        # For several steps delay add this
        if self.local_num %1 == 0:
            previous_weight_s[:] = weight
        self.local_num += 1
        weight[:] += mom
...
#        grad_sync = 1.0*((1-lr*wd)*previous_weight_s - weight)/(4.0)
#        grad_sync = 1.0*((1-lr*wd)*previous_weight_s - weight)/(32.0) # Equal to w(t+1)-w(t-1)/2
	dc_sgd_update(weight, grad, previous_weight_s, out = weight, lr = lr, wd = wd, lam = self.lamda)
#        previous_weight_s[:] = weight
        # For several steps delay add this
        if self.local_num %5 == 0:
            previous_weight_s[:] = weight
        self.local_num += 1
