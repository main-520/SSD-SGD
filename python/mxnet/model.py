...
def _update_params_on_kvstore(param_arrays, grad_arrays, kvstore, param_names, pull_count, num_device, updater, kvlocal):
    ...
    for index, pair in enumerate(zip(param_arrays, grad_arrays)):
        ...
        # Without pull_count, it means training without warmup stage;
#       kvstore.cpweight(name, arg_list, priority=-index)
#        if pull_count > 624:
#            kvstore.cpweight(name, arg_list, priority=-index)
...
# add this 1.4: But it is not really used;
def _update_params_local(param_arrays, grad_arrays, updater, num_device):
    """Perform update of param_arrays from grad_arrays not on kvstore."""
    for i, pair in enumerate(zip(param_arrays, grad_arrays)):
        arg_list, grad_list = pair
        if grad_list[0] is None:
