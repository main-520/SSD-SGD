...
//====================== begin edit this 1.4 ======================//
  ...
  void PullBroadImpl(const std::vector<int>& keys,
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      CopyFromTo(comm_buf_[key], &comm_buf_copy[key]); // edit 4.23;
    }
  }
...
//====================== end edit this 1.4 ======================//
  ...
  void PullImpl(const std::vector<int>& keys,
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      auto &recv_buf_copy= comm_buf_copy[key]; // add this 1.4
...
// This is used for the final 5 iterations; 1.27
/*
...
*/
      // add this 1.4: 199 iterations for warmup
      if(pull_first_time/parameter_count <= 499) {
        CHECK_NOTNULL(Engine::Get())->PushAsync(
                pull_from_servers,
                pinned_ctx_,
                {},
                ...
                "KVStoreDistDefaultStoragePull");
        comm_->Broadcast(key, recv_buf, grouped_vals[i], priority);
        if(pull_first_time/parameter_count > 495)
          CopyFromTo(recv_buf, &recv_buf_copy);
      } else {
//	comm_->Broadcast(key, comm_buf_copy[key], grouped_vals[i], priority);
	int count = pull_first_time/parameter_count;
        if((count>499) && (count%5 == 4)){
        comm_->Broadcast(key, recv_buf_copy, grouped_vals[i], priority);
        CHECK_NOTNULL(Engine::Get())->PushAsync(pull_from_servers,
                                                pinned_ctx_,
                                                {},
                                                {recv_buf.var()},
                                                FnProperty::kNormal,
  ...
  void Push_(const std::vector<int>& keys,
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      auto &local_comm = comm_local_copy[key];// add this 5.20 to store
      auto &update_local = comm_buf_copy[key]; // add this 5.23  to store the comm_buf_copy

      // add this 5.21 malloc the memory for local_comm
      if(local_comm.is_none()){
        local_comm = NDArray(grouped_vals[i][0].shape(), pinned_ctx_, true, grouped_vals[i][0].dtype());
      }
      if(update_local.is_none()){
        update_local = NDArray(grouped_vals[i][0].shape(), pinned_ctx_, true, grouped_vals[i][0].dtype());
      }
...
//*************** add this to update the local weights 5.20 begin ******************

      if (updater_ != nullptr && pull_first_time/parameter_count>499) {
      // This part update the weights locally
        if (key_type_ == kStringKey && str_updater_ != nullptr) {
          const std::string &str_key = reverse_str_key_dict_[key];
          str_updater_(str_key, comm_buf,  &update_local);
        } else {
          updater_(key, comm_buf,  &update_local);
        }
      }

//*************** add this to update the local weights 5.20 end *******************
      // push to servers
      if (storage_type == kDefaultStorage) {
  ...
  */
  ...
  std::unordered_map<int, NDArray> comm_local_copy;



  NDArray count_engine;

  int coung_engine = 0;
  int pull_first_time = 0;
//  int parameter_count = 8;   // Lenet-5
  int parameter_count = 157;   // ResNet-50 
//  int parameter_count = 64; //Resnet-20
//  int parameter_count = 463; //Resnet-152
//  int parameter_count = 278; //Inception-bn
//  int parameter_count = 16;  //Alexnet
//  int parameter_count = 32;  //vgg-16
//  int parameter_count = 60;  //ResNet18_v2
//  int parameter_count = 310;  //resnet101_v2
//  int parameter_count = 83;  //mobilenet1.0
//  int parameter_count = 22;  //vgg11
