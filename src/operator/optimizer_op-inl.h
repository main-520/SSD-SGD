...
// add this 1.4 to support the sqrt operation
#include <cmath>
...
// add this to define the struct for glu-update algorithm Begin

// add this to define the struct for glu-update algorithm End


// add this to define the struct for adc-sgd-update algorithm

//============================ Function for DCASGD Begin ======================
...
struct DCKernel {
    ...
    MSHADOW_XINLINE static void Map(int i, DType* out_data, const DType* weight_data,
                                    ...
                                    const DType param_rescale_grad, const OpReqType req) {
        KERNEL_ASSIGN(out_data[i], 1, (-1.0) * param_lr * (param_wd *weight_data[i] + grad_data[i] +
                (previous_data[i]-weight_data[i])*0.5));
    }
};
...
//============================ Function for DCASGD End   ============================


//============================ Function for MeanSquare calculation Begin add this 1.4===================
...
//============================ Function for MeanSquare calculation End ===================


//============================ Function for calculating the inter of ADCASGD Begin ===================
...
inline void ADCUpdate(const nnvm::NodeAttrs& attrs,
    ...
    MSHADOW_REAL_TYPE_SWITCH(inputs[0].type_flag_, DType, {
            ...
            Tensor<xpu, 2, DType> grad       = inputs[1].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 2, DType> previous   = inputs[2].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 2, DType> MeanSquare = inputs[3].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 2, DType> out = outputs[0].FlatTo2D<xpu, DType>(s);
            Kernel<ADCKernel, xpu>::Launch(s, weight.shape_.Size(), out.dptr_, weight.dptr_,
            grad.dptr_, previous.dptr_, MeanSquare.dptr_, static_cast<DType>(param.clip_gradient),
            static_cast<DType>(param.lr), static_cast<DType>(param.wd),
            static_cast<DType>(param.rescale_grad),
            static_cast<DType>(param.lam), req[0]);
    });
